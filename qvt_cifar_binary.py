...
def train_qvit(n_train, n_test, n_epochs, batch_size, class_pair, class_pair_name, rep_num=0):
    ...
    # CIFAR specific mean and std for denormalization during training batch processing
    cifar_mean_np_train = np.array([0.4914, 0.4822, 0.4465]).reshape(1, 3, 1, 1)
    cifar_std_np_train = np.array([0.2470, 0.2435, 0.2616]).reshape(1, 3, 1, 1)

    model = QSANN_image_classifier(S=S_VALUE_CIFAR, n=N_QUBITS, D=D_QSAL, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE)
    params = init_params(S=S_VALUE_CIFAR, n=N_QUBITS, D=D_QSAL, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE, 
                        input_patch_dim=INPUT_PATCH_DIM_CIFAR, num_classes_out=NUM_CLASSES)
    
    total_params = count_parameters(params)
    print(f"Total number of parameters in the binary QViT model for CIFAR-10 {class_pair_name}: {total_params:,}")
    ...
    for epoch in range(n_epochs):
        ...
        for x_batch_torch, y_batch_torch in train_loader:
            ...
            x_batch_np = (x_batch_np * cifar_std_np_train) + cifar_mean_np_train
            x_batch_np = np.clip(x_batch_np, 0., 1.)
            x_batch_np = x_batch_np.transpose(0, 2, 3, 1) # (B,C,H,W) -> (B,H,W,C)
            x_batch_patches_np = create_patches(x_batch_np)
        ...
        class_labels_for_report = [f'{CIFAR10_CLASSES[class_pair[0]]}', f'{CIFAR10_CLASSES[class_pair[1]]}']
    ...
    print(f"Training completed in {training_time:.2f} seconds for n_train={n_train}, rep {rep_num} (CIFAR {class_pair_name})")
...
n_reps_cifar = 1
train_sizes_cifar = [500, 1000, 2000, 5000, 10000]  # Smaller sizes for binary classification
BATCH_SIZE_CIFAR = 64
...
def run_iterations_for_pair(class_pair_name, class_pair, train_sizes, batch_size, num_epochs):
    ...
    for n_train_current in train_sizes:
        ...
        for rep in range(n_reps_cifar):
            ...
            n_test_int = int(n_train_current // 5)  # Use 1/5 of training size for testing
...
if __name__ == "__main__":
    ...
    # Save results to CSV
    results_df_combined_cifar.to_csv('qvt_cifar_binary_results.csv', index=False)
    print("Results saved to qvt_cifar_binary_results.csv")
    ...
    for class_pair_name in CIFAR_BINARY_PAIRS.keys():
        ...
        legend_elements_cifar = (
            [mpl.lines.Line2D([0], [0], label=f'N={n}', color=colors[i]) for i, n in enumerate(train_sizes_cifar)] +
            [
        ...
        plt.tight_layout()
        plt.savefig(f'qvt_cifar_binary_{class_pair_name}_learning_curves.png')
        plt.close()
        print(f"Plot saved to qvt_cifar_binary_{class_pair_name}_learning_curves.png")
