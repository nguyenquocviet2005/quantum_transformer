...
# ## Binary Classification Experiment for CIFAR-10
import os
import json
import jax
...
def create_experiment_dirs(dataset_name="cifar_binary"):
...
D_PATCH_VALUE = 64  # Reduced from 96 to 64 to lower parameter count
...
# Binary class pairs for CIFAR-10
CIFAR_BINARY_PAIRS = {
...
class QSANN_image_classifier:
    ...
    def __call__(self, x, params):
        # x is initially (batch_size, S, input_patch_dim_actual) e.g. (B, 64, 48) for CIFAR
...
class Adversarial:
    ...
    def run(self, tpe="FGSM", metric=False, fidelity_only=False, **kwargs):
        ...
        if metric:
            ...
            file_name = f"cifar_binary_{tpe}_on_perturbation_magnitude.json"
...
def save_fidelity_metrics(fidelity_data, model_name, dataset_name="cifar_binary"):
...
def save_model_params(params, model_name, dataset_name="cifar_binary"):
...
def load_model_params(model_name, dataset_name="cifar_binary"):
...
def save_model_config(config, model_name, dataset_name="cifar_binary"):
...
def load_model_config(model_name, dataset_name="cifar_binary"):
...
def plot_confusion_matrix(cm, class_names, epoch, n_train, rep_num, d_patch, num_layers, batch_s, class_pair_name, dataset_name="CIFAR"):
    ...
    base_dir = create_experiment_dirs("cifar_binary")
...
def load_cifar_binary_data_raw(n_samples, class_pair, train=False, batch_size=64):
    """Load raw CIFAR-10 data for binary classification without DataLoader."""
...
def load_cifar_binary_data(n_train, n_test, batch_size, class_pair, augment=True):
    """Load and preprocess CIFAR-10 dataset for binary classification with specified class pair."""
    ...
    # Load full CIFAR-10 dataset first
    ...
    print(f"Binary CIFAR-10 dataset loaded: {len(trainset)} train, {len(testset)} test samples for classes {class_pair}")
    return trainloader, testloader

# Parameter Initialization
def init_params(S, n, D, num_layers, d_patch_config, input_patch_dim, num_classes_out=2, key_seed=42):
    ...
    patch_embed_w = jax.random.normal(keys[key_idx], (input_patch_dim, d_patch_config), dtype=jnp.float64) * jnp.sqrt(1.0 / input_patch_dim)
    key_idx += 1
    patch_embed_b = jax.random.normal(keys[key_idx], (d_patch_config,), dtype=jnp.float64) * 0.01
    key_idx += 1

    pos_encoding_params = jax.random.normal(keys[key_idx], (S, d_patch_config), dtype=jnp.float64) * 0.02
    key_idx += 1
    ...
    for i in range(num_layers):
        layer_params = {
            ...
            'ln1_gamma': jnp.ones((d_patch_config,), dtype=jnp.float64),
            'ln1_beta': jnp.zeros((d_patch_config,), dtype=jnp.float64),
            ...
            'ln2_gamma': jnp.ones((d_patch_config,), dtype=jnp.float64),
            'ln2_beta': jnp.zeros((d_patch_config,), dtype=jnp.float64)
        }
...
def train_qvit(n_train, n_test, n_epochs, batch_size, class_pair, class_pair_name, rep_num=0, lipschitz_frequency=1, measure_lipschitz=True,
    ...
    train_loader, test_loader = load_cifar_binary_data(n_train, n_test, batch_size, class_pair, augment=False)
    cifar_mean_np = np.array([0.4914, 0.4822, 0.4465]).reshape(1, 3, 1, 1)
    cifar_std_np = np.array([0.2470, 0.2435, 0.2616]).reshape(1, 3, 1, 1)

    model = QSANN_image_classifier(S=S_VALUE_CIFAR, n=N_QUBITS, D=D_QSAL, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE)
    params = init_params(S=S_VALUE_CIFAR, n=N_QUBITS, D=D_QSAL, num_layers=NUM_LAYERS, d_patch_config=D_PATCH_VALUE, input_patch_dim=INPUT_PATCH_DIM_CIFAR, num_classes_out=NUM_CLASSES)
    
    total_params = count_parameters(params)
    print(f"Total number of parameters in the QViT model for CIFAR binary: {total_params:,}")
    ...
    fidelity_data = None
    if measure_adversarial and n_train == max(train_sizes_cifar):
        ...
        x_test_fid_images, y_test_fid_one_hot = load_cifar_binary_data_raw(
    ...
    if measure_lipschitz and n_train == max(train_sizes_cifar):
    ...
    for epoch in range(n_epochs):
        ...
        # For binary classification, top-5 accuracy is the same as top-1 accuracy
        ...
        lipschitz_info = ""
        if last_batch_x is not None and measure_lipschitz and n_train == max(train_sizes_cifar) and (epoch + 1) % lipschitz_frequency == 0:
        ...
        fidelity_info = ""
        if measure_adversarial and fidelity_data is not None and n_train == max(train_sizes_cifar) and (epoch + 1) % adversarial_frequency == 0:
        ...
        epoch_model_name = f"CIFAR_binary_{class_pair_name}_{n_train}_rep{rep_num}_epoch{epoch+1}"
        save_model_params(current_params, epoch_model_name, "cifar_binary")
        ...
        if epoch == n_epochs -1:
            ...
            plot_confusion_matrix(final_cm, class_labels_for_report, epoch + 1, n_train, rep_num, 
                                  D_PATCH_VALUE, NUM_LAYERS, batch_size, class_pair_name, dataset_name="CIFAR")

    print("--- Saving Trained Parameters ---")

    MODEL_NAME=f"CIFAR_binary_{class_pair_name}_{n_train}_rep{rep_num}"
    save_model_params(current_params, MODEL_NAME, "cifar_binary")
    ...
    save_model_config(model_config, MODEL_NAME, "cifar_binary")
    training_time = time.time() - start_time

    print(f"Training completed in {training_time:.2f} seconds for n_train={n_train}, rep {rep_num} (CIFAR binary {class_pair_name})")

    # Save fidelity data (only for largest training size)
    if epoch_fidelities and n_train == max(train_sizes_cifar):
        fidelity_save_data = {
        ...
        save_fidelity_metrics(fidelity_save_data, MODEL_NAME, "cifar_binary")
    # Save Lipschitz constants data (only for largest training size)
    if n_train == max(train_sizes_cifar):
        base_dir = create_experiment_dirs("cifar_binary")
        lipschitz_file_name = f"qvt_cifar_binary_{class_pair_name}_{n_train}_rep{rep_num}_lipschitz.csv"
...
# Constants for CIFAR binary experiment
n_epochs_cifar = 100  # Binary classification might need more epochs than MNIST
n_reps_cifar = 1
train_sizes_cifar = [10000]  # Smaller sizes for binary classification
BATCH_SIZE_CIFAR = 64
...
def run_iterations_for_pair(class_pair_name, class_pair, train_sizes, batch_size, num_epochs,
                           ...
                           adversarial_frequency=1, measure_adversarial=True):
    """Run multiple training iterations for a given class pair, with fidelity evaluation after each training."""
    all_results_for_pair = []
    adv_results_for_pair = []

    for n_train_current in train_sizes:
        print(f"=== Starting training for CIFAR binary {class_pair_name}: train size {n_train_current}, batch size {batch_size}, epochs {num_epochs} ===")
        for rep in range(n_reps_cifar):
            print(f"\nStarting repetition {rep + 1}/{n_reps_cifar} for {class_pair_name} train size {n_train_current}")
            ...
            if n_train_current == max(train_sizes):
                ...
                x_test_fid_images, y_test_fid_one_hot = load_cifar_binary_data_raw(N_TEST_ADVERSARIAL, class_pair, train=False, batch_size=N_TEST_ADVERSARIAL)
                x_test_fid_patched = create_patches(x_test_fid_images, patch_size=PATCH_SIZE_CIFAR)
                ...
                try:
                    ...
                    trained_params = load_model_params(f"CIFAR_binary_{class_pair_name}_{n_train_current}_rep{rep + 1}", "cifar_binary")
                except Exception:
                    try:
                        # Fallback: try to load from old location
                        import pickle
                        with open(f"CIFAR_binary_{class_pair_name}_{n_train_current}_rep{rep + 1}_trained_params.pkl", "rb") as f:
                ...
                #     print(f"Empirical Lipschitz constant (LC) for CIFAR binary QViT model [{class_pair_name}] (n_train={n_train_current}): {lc_value:.4f}")
                    
                #     # Save LC value
                #     lc_results = {
                #         "class_pair": class_pair_name,
                #         "model_name": f"CIFAR_binary_{class_pair_name}_{n_train_current}_rep{rep + 1}",
                #         "lipschitz_constant_l2": float(lc_value),
                #         "lc_batch_size": lc_batch_size,
                #         "dataset": "CIFAR_binary",
                #         "n_train": n_train_current
                #     }
                #     lc_df = pd.DataFrame([lc_results])
                #     base_dir = create_experiment_dirs("cifar_binary")
                #     lc_csv_name = f"qvt_cifar_binary_{class_pair_name}_{n_train_current}_lipschitz_constant.csv"
    ...
    if adv_results_for_pair:
        ...
        adv_csv_name = f"qvt_cifar_binary_{class_pair_name}_adversarial_results.csv"
...
if __name__ == "__main__":
    all_results_collected_cifar = []
    
    for class_pair_name, class_pair in CIFAR_BINARY_PAIRS.items():
        print(f"\n{'='*50}")
        print(f"Starting experiments for CIFAR binary pair: {class_pair_name} (classes {class_pair}: {CIFAR10_CLASSES[class_pair[0]]} vs {CIFAR10_CLASSES[class_pair[1]]})")
        print(f"{'='*50}")
        
        results = run_iterations_for_pair(class_pair_name, class_pair, train_sizes_cifar, BATCH_SIZE_CIFAR, n_epochs_cifar,
                                         lipschitz_frequency=1, measure_lipschitz=True,
                                         adversarial_frequency=1, measure_adversarial=False)
        all_results_collected_cifar.append(results)

    # Combine all results
    results_df_combined_cifar = pd.concat(all_results_collected_cifar, ignore_index=True)

    # Save results to organized directory structure
    base_dir = create_experiment_dirs("cifar_binary")
    results_path = os.path.join(base_dir, "metrics", "qvt_cifar_binary_results.csv")
    results_df_combined_cifar.to_csv(results_path, index=False)
    print(f"Results saved to {results_path}")

    # Generate aggregated results and plots for each class pair
    df_agg_cifar = results_df_combined_cifar.groupby(["class_pair", "n_train", "batch_size", "step"]).agg({
        "train_cost": ["mean", "std"],
        "test_cost": ["mean", "std"],
        "train_acc": ["mean", "std"],
        "test_acc": ["mean", "std"],
    ...
    for class_pair_name in CIFAR_BINARY_PAIRS.keys():
        df_pair = df_agg_cifar[df_agg_cifar.class_pair == class_pair_name]
        
        colors = sns.color_palette("plasma", n_colors=len(train_sizes_cifar))
        fig, axes = plt.subplots(ncols=3, figsize=(16.5, 5))
        generalization_errors_cifar = []

        for i, n_train in enumerate(train_sizes_cifar):
            df = df_pair[(df_pair.n_train == n_train) & (df_pair.batch_size == BATCH_SIZE_CIFAR)]
            if df.empty:
                continue
            ...
            for k in range(4):
                ax = axes[axs[k]]
                ax.plot(df.step, dfs[k], lines[k], label=labels[k], markevery=max(1, n_epochs_cifar // 10), color=colors[i], alpha=0.8)

            # Compute generalization error at the end of training
            if not df[df.step == n_epochs_cifar].empty:
                train_cost_final = df[df.step == n_epochs_cifar].train_cost["mean"].values[0]
                test_cost_final = df[df.step == n_epochs_cifar].test_cost["mean"].values[0]
                dif = test_cost_final - train_cost_final
                generalization_errors_cifar.append(dif)
            else:
                generalization_errors_cifar.append(np.nan)

        axes[0].set_title(f'Train and Test Losses (CIFAR {class_pair_name})', fontsize=14)
        axes[0].set_xlabel('Epoch')
        axes[0].set_ylabel('Loss')

        axes[1].plot(train_sizes_cifar, generalization_errors_cifar, "o-", label=r"$gen(\alpha)$")
        axes[1].set_xscale('log')
        axes[1].set_xticks(train_sizes_cifar)
        axes[1].set_xticklabels([str(ts) for ts in train_sizes_cifar])
        axes[1].set_title(fr'Generalization Error (CIFAR {class_pair_name})', fontsize=14)
        axes[1].set_xlabel('Training Set Size')
        axes[1].set_yscale('log', base=2, nonpositive='clip')

        axes[2].set_title(f'Train and Test Accuracies (CIFAR {class_pair_name})', fontsize=14)
        axes[2].set_xlabel('Epoch')
        axes[2].set_ylabel('Accuracy')
        axes[2].set_ylim(0.5, 1.05)  # Binary classification accuracies

        legend_elements_cifar = (
            [plt.Line2D([0], [0], label=f'N={n}', color=colors[i]) for i, n in enumerate(train_sizes_cifar)] +
            [
                plt.Line2D([0], [0], marker='o', ls='-', label='Train', color='Black'),
                plt.Line2D([0], [0], marker='x', ls='--', label='Test', color='Black')
            ]
        )

        axes[0].legend(handles=legend_elements_cifar, ncol=2)
        axes[2].legend(handles=legend_elements_cifar, ncol=2)

        plt.tight_layout()
        # Save to organized directory structure
        base_dir = create_experiment_dirs("cifar_binary")
        plot_path = os.path.join(base_dir, "plots", f'qvt_cifar_binary_{class_pair_name}_learning_curves.png')
        plt.savefig(plot_path)
        plt.close()
        print(f"Plot saved to {plot_path}")

    print("CIFAR binary QViT script setup complete.")
    ...
    #     try:
    #         import pickle
    #         params = load_model_params(f"CIFAR_binary_{class_pair_name}_100_rep1", "cifar_binary")
    #     except Exception:
