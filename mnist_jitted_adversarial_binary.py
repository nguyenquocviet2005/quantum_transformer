# qvt_mnist_adversarial_binary.py
...
NUM_LAYERS = 1
PATCH_SIZE_MNIST = 4
S_VALUE_MNIST = (28 // PATCH_SIZE_MNIST)**2
INPUT_PATCH_DIM_MNIST = (PATCH_SIZE_MNIST**2) * 1
N_QUBITS = 5
D_QSAL = 1
...
def load_mnist_binary_data(n_train, n_test_val, batch_size, class_pair, augment=False):
    ...
    trainloader = torch.utils.data.DataLoader(binary_trainset, batch_size=batch_size, shuffle=True, num_workers=0)
    testloader = torch.utils.data.DataLoader(binary_testset, batch_size=batch_size, shuffle=False, num_workers=0)
...
class Adversarial:
    ...
    def run(self, tpe="FGSM", metric=False, fidelity_only=False, **kwargs):
        ...
        # Optionally jit the core computation (not the full run method)
        # _run_core_jit = jax.jit(_run_core, static_argnames=())
        # But since it uses self, we keep it as a regular function
...
def train_single_qvt_model(
    ...
    measure_lipschitz=True, lipschitz_frequency=1
):
    """
    Train a single QVT model for binary MNIST classification.
    ...
    """
    train_loader, val_loader = load_mnist_binary_data(n_train, n_test_val, batch_size, class_pair, augment=augment_data)
    ...
    if measure_fidelity:
        ...
        x_test_adv_images, y_test_adv_one_hot = load_mnist_binary_data_raw(
...
def run_binary_adversarial_experiments():
    """Run adversarial experiments for all MNIST binary pairs"""
    results_all_pairs = []
    
    for pair_name, class_pair in MNIST_BINARY_PAIRS.items():
        print(f"\n=== Running experiments for pair {pair_name}: {class_pair} ===")
        
        # Model name for this specific pair
        pair_model_name = f"{MODEL_NAME}_{pair_name}_10000_rep1"
        ...
        if TRAIN_NEW_MODEL:
            ...
            # Save fidelity data during training (similar to mnist_jitted_adversarial.py)
            ...
            # This contains empirical Lipschitz constants computed from gradient norms
            # across training epochs, providing insights into model stability
            ...
            # Save model for this pair
            save_model_params(trained_qvt_params, pair_model_name)
            model_config = {
                'S': S_VALUE_MNIST, 'n_qubits': N_QUBITS, 'D': D_QSAL,
                'num_layers': NUM_LAYERS, 'd_patch': D_PATCH_VALUE,
                'input_patch_dim': INPUT_PATCH_DIM_MNIST, 'num_classes': NUM_CLASSES,
                'patch_size': PATCH_SIZE_MNIST, 'class_pair': class_pair
            }
        ...
        pair_csv_name = f"qvt_mnist_binary_{pair_name}_adversarial_results.csv"
    ...
    print(results_df)
    results_df.to_csv("qvt_mnist_binary_all_adversarial_results.csv", index=False)
    print("Combined adversarial results saved to qvt_mnist_binary_all_adversarial_results.csv")
    
    return results_df

# --- Main Execution ---
if __name__ == "__main__":
    print("Starting MNIST Binary Adversarial Experiments")
    print(f"Binary pairs: {MNIST_BINARY_PAIRS}")
